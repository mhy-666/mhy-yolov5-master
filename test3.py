...
class YOLOV5_ONNX(object):
    ...
    def plot_one_box(self,x, img, color=None, label=None, line_thickness=None):
        ...
        # print(img1.dtype)
        img2=img.copy()
        cv2.rectangle(img2, c1, c2, color,thickness=tl, lineType=cv2.LINE_AA)
        if label:
            tf = max(tl - 1, 1)  # font thickness
            t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
            c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
            cv2.rectangle(img2, c1, c2, color, -1, cv2.LINE_AA)  # filled
            cv2.putText(img2, label, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
